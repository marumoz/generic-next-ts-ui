...
const DatePickerInput = (props: InputProps) => {
    ...
    useEffect(() => {
        if(!value){
            updateText(inputId, formId, { textInput: moment().format('YYYY-MM-DD') });
        }
        if(minDate === 'above18'){
    ...
    const handleChangeText = (date: string, dateString: string | string[]) => {
        if(dateString){
